-- Add DEX tracking fields to machines table (FIXED VERSION)
...
-- Check if dex_captures table exists and has the expected columns
-- If the view creation fails, run this to see what columns exist:
-- SELECT column_name FROM information_schema.columns WHERE table_name = 'dex_captures';

-- Create a simpler view that works regardless of exact column structure
...
LEFT JOIN (
  SELECT
    ...
    MAX(COALESCE(capture_time, created_at)) as last_capture_actual,
    COUNT(*) FILTER (WHERE COALESCE(capture_time, created_at) >= NOW() - INTERVAL '24 hours') as last_24hrs,
    COUNT(*) FILTER (WHERE COALESCE(capture_time, created_at) >= NOW() - INTERVAL '4 hours') as last_4hrs_actual,
