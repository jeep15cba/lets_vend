  ...
  for (const line of lines) {
    ...
    try {
      ...
      switch (segmentType) {
        ...
        case 'MA5':
          // Support all temperature field variations
          if (segments[1] && segments[1].toUpperCase().includes('TEMP')) {
            const rawTemp = segments[2]?.trim();
            const tempUnit = segments[3];

            // Convert temperature: divide by 10 for most machines (e.g., 50 = 5.0°C)
            // Values > 100 might need division by 100 (e.g., 400 = 4.0°C)
            let convertedTemp = null;
            if (rawTemp) {
              const tempValue = parseFloat(rawTemp);
              if (!isNaN(tempValue)) {
                // If value > 100, divide by 100; otherwise divide by 10
                convertedTemp = tempValue > 100 ? (tempValue / 100).toFixed(1) : (tempValue / 10).toFixed(1);
              }
            }

            if (segments[1].toUpperCase().includes('DESIRED')) {
              keyValue['desired_temperature'] = convertedTemp;
              keyValue['desired_temperature_unit'] = tempUnit;
            } else if (segments[1].toUpperCase().includes('DETECTED')) {
              keyValue['detected_temperature'] = convertedTemp;
              keyValue['detected_temperature_unit'] = tempUnit;
            } else if (segments[1].toUpperCase() === 'TEMP' ||
                       (segments[1].toUpperCase().includes('TEMP') && !keyValue['detected_temperature'])) {
              // Generic TEMP field - only use if we don't already have detected temperature
              keyValue['detected_temperature'] = convertedTemp;
              keyValue['detected_temperature_unit'] = tempUnit;
            }
